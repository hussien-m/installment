...
class DashboardController extends Controller
{
    ...
    public function customerMessages()
    {
        ...
        $data['messages'] = Messages::with('customer')->latest()->get();
    ...
    public function replayMessage(Request $request , $id)
    {
       ...
       try{
        ...
        $replay->replay_message = $request->replay_message;
        $replay->message_id = $msg->id;
        $replay->save();

        \DB::commit();
 
        session()->flash('message','Replay Success!');
        session()->flash('type','success');
        return redirect()->back();

       }catch(Exception $ex){
        \DB::rollback();
        session()->flash('message','No more than one reply is allowed');
